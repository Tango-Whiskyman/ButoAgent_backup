...
@click.option('--test_pull_name', default='main', help='the name of the test pull')
...
def main(container_name: str, port: int, test_pull_name: str, git_clone: bool, local_env: bool):
    ...
    while True:
        ...
        mode = single_select_menu(['user mode', 'exit'], "Please select the mode:")
        match mode:
            case 'user mode':
                clear_screen()
...
def user_mode(model: str, context_variables: dict, debug: bool = True): 
    ...
    agents = {task_manager_agent.name.replace(' ', '_'): task_manager_agent}
    for agent_name in task_manager_agent.agent_teams.keys():
        agents[agent_name.replace(' ', '_')] = task_manager_agent.agent_teams[agent_name]("placeholder").agent
