...
from autoagent.agents.answer_validator_agent import get_answer_validator_agent
from autoagent.agents.task_manager_agent import get_task_manager_agent
from autoagent.agents.web_retriever_agent import get_web_retriever_agent
...
def get_system_triage_agent(model: str, **kwargs):
    ...
    task_manager_agent = get_task_manager_agent(model)
...
"""
    ...
    def transfer_to_answer_validator_agent(sub_task_description: str):
        return Result(value=sub_task_description, agent=answer_validator_agent)
    def transfer_to_task_manager_agent(sub_task_description: str):
        return Result(value=sub_task_description, agent=task_manager_agent)
    def transfer_to_web_retriever_agent(sub_task_description: str):
        return Result(value=sub_task_description, agent=web_retriever_agent)
    ...
    def transfer_back_to_manager_agent(task_status: str):
        """
        Args:
            task_status: The detailed description of the task status after a sub-agent has finished its sub-task. A sub-agent can use this tool to transfer the conversation back to the `Task Manager Agent` only when it has finished its sub-task.
        """
        return Result(value=task_status, agent=task_manager_agent)
    system_triage_agent.agent_teams = {
        ...
        answer_validator_agent.name: transfer_to_answer_validator_agent,
        task_manager_agent.name: transfer_to_task_manager_agent,
        web_retriever_agent.name: transfer_to_web_retriever_agent
    }
    system_triage_agent.functions.extend([transfer_to_filesurfer_agent, transfer_to_websurfer_agent, transfer_to_coding_agent, transfer_to_answer_validator_agent, transfer_to_task_manager_agent, transfer_to_web_retriever_agent])
    ...
    answer_validator_agent.functions.append(transfer_back_to_manager_agent)
    task_manager_agent.functions.append(transfer_back_to_manager_agent)
    web_retriever_agent.functions.append(transfer_back_to_manager_agent)
